...
export class AiService {
  ...
  constructor() {
    ...
    if (!key) {
      console.warn('GENAI_API_KEY not set. AI calls will fail until you set the key.');
    } else {
      this.genai = new GoogleGenerativeAI(key); // <-- pass string directly
    }
  }
...
Context: ${JSON.stringify(context).slice(0, 2000)}`;
      ...
      return response.response.text() || "I'm here to listen. Could you tell me more about how you're feeling?";
    } catch (error) {
  ...
  async generateJournalInsights(journalEntries: any[], moodData: any[]) {
    ...
    try {
      ...
      try {
        ...
        return {
          insights: result.insights || ['Your mood tracking shows commitment to self-awareness.'],
          recommendations: result.recommendations || ['Continue your daily mood tracking practice.'],
          patterns: result.patterns || ['Regular journaling appears to support your wellbeing.'],
        };
      } catch {
        return {
